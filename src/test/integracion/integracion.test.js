...
describe('Pruebas del HTML de BuscaMascotas', () => {
  ...
  test('verifica que haya un enlace a "MASCOTA PERDIDA"', () => {
    document.body.innerHTML = html; // Inyecta el HTML en el DOM
    const homeLink = screen.getByText(/MASCOTA PERDIDA/i); // Busca el enlace
    expect(homeLink).toBeInTheDocument(); // Verifica que el enlace esté en el documento
  });

  test('verifica que haya un enlace a "MASCOTAS ENCONTRADAS"', () => {
    document.body.innerHTML = html; // Inyecta el HTML en el DOM
    const homeLink = screen.getByText(/MASCOTAS ENCONTRADAS/i); // Busca el enlace
    expect(homeLink).toBeInTheDocument(); // Verifica que el enlace esté en el documento
  });
